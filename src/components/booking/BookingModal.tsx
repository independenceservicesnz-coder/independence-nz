          ...
          <div className="bg-white rounded-t-3xl md:rounded-2xl w-full max-w-lg shadow-2xl max-h-[95vh] overflow-y-auto">
            ...
            {step === 'processing' && (
              <div className="p-12 text-center">
                ...
                <p className="font-serif text-xl font-medium text-gray-800 mb-2">Setting up your booking...</p>
                <p className="text-sm text-gray-400">Taking you to secure payment. Please do not close this page.</p>
              </div>
            )}
              ...
              <>
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mt-0.5">{serviceName} · <span className="text-brand-500 font-semibold">${price} NZD</span></p>
                  </div>
                  <button onClick={closeModal} className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-500 text-sm">✕</button>
                </div>
                <div className="p-5">
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-5">
                    {SLOTS.map((s, i) => (
                      <button
                        ...
                        className={`p-3 rounded-xl border-2 text-left transition-all ${selectedSlot === i ? 'border-brand-400 bg-brand-50' : 'border-gray-100 hover:border-brand-200 bg-white'}`}
                      >
                        <p className={`text-sm font-semibold ${selectedSlot === i ? 'text-brand-600' : 'text-gray-700'}`}>{s.display}</p>
                        <p className={`text-xs mt-0.5 ${selectedSlot === i ? 'text-brand-500' : 'text-gray-400'}`}>{s.time_label}</p>
                      </button>
                    ))}
                  </div>
                  ...
                  <button onClick={() => setStep('details')} className="w-full bg-brand-500 hover:bg-brand-600 text-white font-semibold py-4 rounded-xl transition-colors text-sm">
                    Continue →
                  </button>
            ...
            {step === 'details' && (
              <>
                <div className="flex items-center gap-3 p-5 border-b border-gray-100">
                  <button onClick={() => setStep('slots')} className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-500 text-sm">←</button>
                  <div className="flex-1">
                    <h3 className="font-serif text-xl font-medium">Confirm booking</h3>
                    <p className="text-sm text-gray-400 mt-0.5">{SLOTS[selectedSlot].display} · {SLOTS[selectedSlot].time_label}</p>
                  </div>
                  <button onClick={closeModal} className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-500 text-sm">✕</button>
                </div>
                <div className="p-5 space-y-4">
                  <div className="bg-gray-50 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        ...
                        <p className="text-xs text-brand-500 mt-0.5">{SLOTS[selectedSlot].display} · {SLOTS[selectedSlot].time_label}</p>
                      </div>
                  ...
                  <button
                    ...
                    className="w-full bg-brand-500 hover:bg-brand-600 text-white font-bold py-4 rounded-xl transition-colors text-base flex items-center justify-center gap-2"
                  >
                    {loading ? (
                      <>
                  ...
                  <p className="text-xs text-gray-400 text-center">
                    Prefer to book by phone? <a href="tel:0273259707" className="text-brand-500 font-medium">Call 027 325 9707</a>
                  </p>
                </div>
              </>
            )}
