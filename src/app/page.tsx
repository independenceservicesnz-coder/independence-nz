  ...
  return (
    <div className="min-h-screen bg-[#F7F9F7] flex flex-col">
      ...
      <div className="max-w-lg mx-auto px-4 py-10 w-full">
        ...
        <h1 className="font-serif text-3xl font-medium mb-2">Book {providerName}</h1>
        <p className="text-gray-400 text-sm mb-8">{serviceName} · <span className="text-brand-500 font-semibold">${price} NZD</span></p>

        {/* Not signed in warning */}
        {!user && (
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6">
            <p className="text-sm font-medium text-blue-700 mb-1">👤 You need to sign in to book</p>
            <p className="text-xs text-blue-600 mb-3">Create a free account or sign in to continue.</p>
            <Link href={'/auth/login?redirect=' + encodeURIComponent('/book?provider=' + encodeURIComponent(providerName) + '&service=' + encodeURIComponent(serviceName) + '&price=' + price)}
              className="text-sm font-semibold text-white bg-brand-500 hover:bg-brand-600 px-4 py-2 rounded-lg inline-block transition-colors">
              Sign in to book →
            </Link>
          </div>
        )}
        ...
        <button
          type="button"
          onClick={handlePay}
          className="w-full bg-brand-500 hover:bg-brand-600 text-white font-bold py-5 rounded-xl transition-colors text-lg flex items-center justify-center gap-3 shadow-lg"
        >
          {loading ? (
            <>
        ...
        <p className="text-xs text-gray-400 text-center mt-3">
          Secure payment by Stripe · Need help?{' '}
