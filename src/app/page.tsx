...
const providers = [
  { id: '1', name: 'Pristine Home Services', cat: 'Home Cleaning', location: 'Auckland Central', rating: 4.9, reviews: 128, price: 85, priceLabel: '$85 / visit', avail: 'Available tomorrow', emoji: '🧹', bg: 'from-emerald-50 to-emerald-100', topRated: true },
  { id: '2', name: 'Green Thumb Gardens', cat: 'Gardening', location: 'North Shore', rating: 4.8, reviews: 94, price: 65, priceLabel: '$65 / hour', avail: 'Available Friday', emoji: '🌿', bg: 'from-emerald-50 to-amber-50', topRated: false },
  { id: '3', name: 'Reliable Handyman Co.', cat: 'Handyman', location: 'Manukau', rating: 4.7, reviews: 61, price: 75, priceLabel: '$75 / hour', avail: 'Available Monday', emoji: '🔧', bg: 'from-violet-50 to-purple-100', topRated: false },
  { id: '4', name: 'MoveBetter Health', cat: 'Fitness & Mobility', location: 'Remuera', rating: 5.0, reviews: 47, price: 90, priceLabel: '$90 / session', avail: 'Available today', emoji: '🏃', bg: 'from-rose-50 to-pink-100', topRated: true },
]
...
const howItWorks = [
  ...
  { n: '3', title: 'Book and pay — no extra fees', desc: 'You only pay for the service itself. No platform fees, no membership fees, no hidden costs. Ever.' },
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="bg-gradient-to-br from-brand-700 via-brand-500 to-brand-400 text-white">
        <div className="max-w-6xl mx-auto px-4 py-20 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-10">
            <Link href="/browse" className="bg-white text-brand-500 font-semibold px-8 py-3.5 rounded-xl hover:bg-gray-50 transition-colors text-base w-full sm:w-auto text-center">
              Browse Services →
            </Link>
            <Link href="/providers" className="border border-white/40 text-white px-8 py-3.5 rounded-xl hover:bg-white/10 transition-colors text-sm w-full sm:w-auto text-center">
              Become a Provider
            </Link>
          </div>
      ...
      <div className="bg-brand-500 text-white">
        <div className="max-w-6xl mx-auto px-4 py-5 text-center">
          <p className="font-serif text-xl font-medium mb-1">You only ever pay for the service you book. Nothing more.</p>
          <p className="text-white/75 text-sm">No membership fees · No platform fees · No hidden charges · Just the cost of your service</p>
        </div>
      </div>
      ...
      <div className="bg-brand-700 text-white">
        <div className="max-w-6xl mx-auto px-4 py-4 flex flex-col md:flex-row items-center justify-center gap-4 text-center md:text-left">
          ...
          <a href="tel:0273259707" className="shrink-0 bg-white text-brand-500 font-bold px-6 py-2.5 rounded-xl hover:bg-gray-50 transition-colors text-sm">
            📞 027 325 9707
          </a>
      ...
      <section className="max-w-6xl mx-auto px-4 py-16 w-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {categories.map(c => (
            <Link key={c.slug} href={`/browse?cat=${c.slug}`}
              className={`bg-white rounded-xl border-2 p-5 text-center hover:shadow-md transition-all group ${c.featured ? 'border-brand-400' : 'border-gray-100 hover:border-brand-200'}`}>
              {c.featured && <div className="text-xs font-medium text-brand-500 bg-brand-50 px-2 py-0.5 rounded-full inline-block mb-2">Popular</div>}
          ...
          {providers.map(p => (
            <div key={p.id} className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all group flex flex-col">
              ...
              <div className="p-4 flex flex-col flex-1">
                <p className="font-medium text-gray-900 text-sm mb-0.5">{p.name}</p>
                ...
                <div className="mt-auto pt-3 border-t border-gray-50">
                  <div className="flex items-center gap-1 mb-2">
                    ...
                    <span className="text-xs text-brand-500 font-medium">{p.avail}</span>
                  </div>
                  <Link href={`/book?provider=${encodeURIComponent(p.name)}&service=${encodeURIComponent(p.cat)}&price=${p.price}&providerId=${p.id}`}
      ...
      <section className="max-w-6xl mx-auto px-4 py-16 w-full">
        <div className="rounded-2xl overflow-hidden h-72 md:h-96 relative">
          <img
            ...
            alt="Happy elderly person smiling"
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16 w-full">
        <div className="bg-gradient-to-r from-brand-700 to-brand-500 rounded-2xl p-10 flex flex-col md:flex-row items-center justify-between gap-6 text-white">
          ...
          <div className="shrink-0 text-center">
            <Link href="/providers" className="bg-white text-brand-500 font-semibold px-8 py-3.5 rounded-xl hover:bg-gray-50 transition-colors text-sm inline-block">
              Become a Provider
            </Link>
